...
## notify — Codex / Claude Code / Gemini CLI 完成一轮输出时响铃提醒

让 AI 跑完当前回答时发出提示音，方便你及时回来检查结果。原理是给各个 CLI 挂一个
"完成事件" hook，触发时播放一段系统提示音：

- **Codex** → `~/.codex/config.toml` 的用户级 `notify`（收到通知时触发）
...
2. 把通知配置合并进 Codex 的 `config.toml`，并把 hook 合并进 Claude Code 的 `settings.json`（`Stop`）和 Gemini CLI 的 `settings.json`（`AfterAgent`），
   保留原有配置，写回时不带 BOM（避免 JSON 解析失败）
3. 只对检测到的 CLI 生效（对应配置目录存在才改）
...
> Codex 的 `notify` 是用户级配置，不能放进项目内的 `.codex/config.toml`。同样是 OS / CLI 层面配置，不随账号同步，跨设备靠本仓库 + 每台机器跑一次安装脚本。
